...
def _run_with_exec(src_code: str) -> Union[str, Exception]:
    ...
    try:
        ...
        if status != GRB.OPTIMAL:
            ...
            ans = "Optimal number of parking stops skipped:" + str(round(locals_dict["percentage_skipped"],2)) +"%\n"+ \
                  str(locals_dict["percentage_skipped_each_route"]) + \
                  "Number of stops that are only passed once:"+ str(locals_dict["count_1"]) + \
                  "Number of stops that are passed less than three times:" + str(locals_dict["count_3"])
